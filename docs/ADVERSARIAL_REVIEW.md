...
> | V-1 rigid registration across SfM scales | Fixed. Registration always happens in the canonical frame, never between reconstruction frames. A re-scan calibrated to the same surveyed datum is placed directly and only verified. Anything else gets a full similarity (FPFH matches, numpy RANSAC with a ±10 % scale prior, similarity ICP). Every alignment must pass physical gates: correspondences, inlier ratio, scale, residual rigid motion, ICP residual, confidence. A failure is `ALIGNMENT_REJECTED`: a terminal version status, nothing merged, parent untouched. This is tested on synthetic clouds in CI, with real FPFH in the Open3D image (5/5), and B5 re-run. **No real venue re-scan.** |
> | V-2 splice appends the whole region | Fixed. The splice replaces a volume (polygon × the re-scan's height range): unrelated geometry above or below is kept, and the re-scan is cropped to the volume. The seam step is measured and a visible seam is refused. The exact removed and added indices are published (`SPLICE_INDEX`) and recorded on the version. |
...
> | R-3 splat colour space | Fixed. Training renders `SH_C0 · f_dc + 0.5`, the value the PLY, the ksplat and the viewer use (`chaya_worker.splat_color`). A deterministic fixture takes known RGB from SfM colours to viewer bytes (within one byte, never brighter). A converged CPU fit renders from the decoded ksplat within 0.02 of its target. With the old sigmoid reinstated, these tests fail. |
> | R-4 no densification | Addressed in code: clone, split, opacity and size pruning, opacity reset, and a Gaussian cap, with Adam state carried. Tests show the count changes during training. **Validated on CPU with a test renderer only.** gsplat training has never run (no GPU here), and nothing has been measured on a real scene (no PSNR). |
> | R-5 time budget / PARTIAL dead path | Addressed. The loop stops before the deadline with a margin. It publishes `SPLAT_PARTIAL` (`partial=true`) and a resumable `SPLAT_CHECKPOINT` under `TIME_LIMIT_EXCEEDED`, which the control plane turns into a PARTIAL run. This is tested through the orchestrator with a real deadline. The control plane does not yet hand a checkpoint to a new job, so resume is worker-side only. |
