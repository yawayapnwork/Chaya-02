...
| REGION_ALIGNMENT | parent canonical frame + region metric scale | `NOT_CALIBRATED`. Registration runs in the canonical frame: DIRECT_CANONICAL when both are surveyed to the venue control points, otherwise a full similarity (FPFH + RANSAC + ICP). A scale correction beyond `alignment_max_scale_correction` (10 %), or any other failed quality gate, is `ALIGNMENT_REJECTED` ([rescan.md](rescan.md)). |
