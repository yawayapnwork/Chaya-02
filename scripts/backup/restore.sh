...
if [ "$what" = minio ]; then
  ...
  # Check every bucket before touching any, so a missing mirror cannot leave a half-restored store.
  for bucket in "$S3_BUCKET_RAW" "$S3_BUCKET_DERIVED"; do
    [ -d "$BACKUP_DIR/minio/$bucket" ] || { echo "no mirror for $bucket in $BACKUP_DIR/minio; nothing restored" >&2; exit 1; }
  done
  for bucket in "$S3_BUCKET_RAW" "$S3_BUCKET_DERIVED"; do
