...
## 7a. Coordinate frames
Every stored position -- POIs, AR anchor poses, navigation graphs, route waypoints, re-scan regions -- is in the canonical venue frame: metres, right-handed, +Z up (opposite gravity), origin fixed by the frame's horizontal datum. A reconstruction's own frame (COLMAP/GLOMAP output) has arbitrary scale, rotation and origin; the only way from it to metres is a calibrated, versioned similarity transform `X_canonical = s·R·X_reconstruction + t` in `coordinate_frame`, owned by the control plane and built from measured references (at least two measured distances, or at least three surveyed control points) plus a real gravity source (the reconstructed floor plane, operator floor points, or the control points). Without it, metric consumers report `NOT_CALIBRATED` instead of assuming metres. Recast's +Y-up axes and AR devices' +Y-up tracking frames are converted at one named boundary each. The normative description is [docs/coordinate-frames.md](docs/coordinate-frames.md).
...
## 9. AR clients
Android: WebXR in the Next.js app. iOS: native Swift/ARKit app (WebXR unsupported on iOS Safari). Both use the same backend path-data API (path polyline in venue frame + anchor list). Localization: multi-anchor fiducial relocalization gives a venue-frame transform; VIO interpolates between fixes; residual disagreement between anchors is reported and drift beyond a threshold prompts relocalization. Coordinates are always real: anchors are registered in the floor's current canonical coordinate frame (§7a), device poses arrive in the device's gravity-aligned +Y-up convention and the server converts them at one boundary. See [docs/ar.md](docs/ar.md) for the anchor data model, the relocalization math, tracking-loss handling, and how each client is structured.
